...
clc
%%
data = load('YasiEpochAVGRefICA.txt');
%%
data = reshape(data,19,34000,11);
% data = Data;
%% Total Powers
fs = 200;
TotalPowers = zeros(19,11);
for ch = 1:19
    for trial = 1:11
...
%% Band Powers
fs = 200;
DeltaPowers = zeros(19,11);
ThetaPowers = zeros(19,11);
AlphaPowers = zeros(19,11);
BetaPowers = zeros(19,11);
GamaPowers = zeros(19,11);
for ch = 1:19
    for trial = 1:11
...
%     subplot(10,1,i)
    ch = i;
    plot(TotalPowers(ch,:),'LineWidth',1.5);
    hold on
...
%% Band Power Avg Over All Channels
...
%%
tmp01 = [MaryamDelta ; ones(1,11)];
tmp01(1,:) = tmp01(1,:) / max(MaryamDelta) * 20;
tmp02 = [AliDelta ; 2*ones(1,11)];
tmp02(1,:) = tmp02(1,:) / max(AliDelta) * 20;

%% 
scatter(tmp01(2,1:2:11),tmp01(1,1:2:11),'b')
hold on
scatter(tmp01(2,2:2:10),tmp01(1,2:2:10),'r')
hold on
scatter(tmp02(2,1:2:11),tmp02(1,1:2:11),'b')
hold on
scatter(tmp02(2,2:2:10),tmp02(1,2:2:10),'r')
