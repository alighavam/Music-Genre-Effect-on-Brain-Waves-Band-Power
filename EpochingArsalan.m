...
function EpochedData = epoch(Data, Fs) 
    ...
    TrialsIndex = [TrialsIndex(1:8) ; 742070 ; TrialsIndex(9:end)];
    
    TrialsIndex(18:end) = []; % Deleting Alam Task
    
    a = [1,2,4,5,7,8,9,10,12]; % Triggers that are related to a real trial
    TrialsIndex = TrialsIndex(a); % index of first time point of each trial
    TrialsIndex
    
    EpochedData = zeros(19, 85000, 9); % (Channel(= 19), Time(= 170s), Trial(= 9))
    for ch = 1:19
        for trial = 1:9
